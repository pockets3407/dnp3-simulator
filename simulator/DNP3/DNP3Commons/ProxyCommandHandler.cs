...
namespace Automatak.Simulator.DNP3.Commons
{
    ...
    public class ProxyCommandHandler : ICommandHandler
    {
        ...
        CommandStatus ICommandHandler.Operate(ControlRelayOutputBlock command, ushort index, IDatabase database, OperateType opType)
        {
            lock (mutex)
            {
                return GetOrElseAndLogBinary(command, index, binaryMap, () => proxy.Operate(command, index, database, opType));
            }
        }
        ...
        CommandStatus ICommandHandler.Operate(AnalogOutputInt32 command, ushort index, IDatabase database, OperateType opType)
        {
            lock (mutex)
            {
                return GetOrElseAndLogAnalog(command.value, index, analogMap, () => proxy.Operate(command, index, database, opType));
            }
        }
